...
import net.alfheim.internal.kotlinui.extensions.TextRendererExtensionsKt;
...
public class GeometryUtils {
    ...
    public static void renderString(DrawContext context, Color color, int x, int y, int maxWidth, Text text) {
        final TextRenderer renderer = client.textRenderer;

        final Style style = text.getStyle();
        final Deque<StringVisitable> lines = new ArrayDeque<>(
                TextRendererExtensionsKt.javaWrapLinesWithStyle(renderer, text, maxWidth)
        );

        int height = 0;
        while (lines.size() > 0) {
            final StringVisitable line = lines.pollFirst();

            context.drawText(
                    renderer,
                    Text.literal(line.getString()).setStyle(style),
                    x,
                    y + height,
                    color.getRGB(),
                    false
    ...
    public static void renderLine(MatrixStack stack, Color color, double x, double y, double x1, double y1) {
        float[] colorFloat = intArrayToFloatArray(ARGBIntToRGBA(color.getRGB()));
